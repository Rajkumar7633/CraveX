...
func (h *OrderHandler) GetUserOrders(c *gin.Context) {
	...
	orders, err := h.orderService.GetUserOrders(userUUID)
	if err != nil {
...
func (h *OrderHandler) GetRiderOrders(c *gin.Context) {
	...
	riderUUID, err := uuid.Parse(riderID.(string))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid rider ID"})
		return
	}

	orders, err := h.orderService.GetRiderOrders(riderUUID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *OrderHandler) CancelOrder(c *gin.Context) {
	...
	if err := h.orderService.CancelOrder(id, req.Reason, userUUID); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *OrderHandler) AssignRider(c *gin.Context) {
	id, err := uuid.Parse(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid order ID"})
		return
	...
	if err := h.orderService.AssignRider(id, req.RiderID); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
