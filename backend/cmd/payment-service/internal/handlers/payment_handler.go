...
func NewPaymentHandler(paymentService services.PaymentService, walletService services.WalletService) *PaymentHandler {
...
func (h *PaymentHandler) GetUserPayments(c *gin.Context) {
	...
	payments, err := h.paymentService.GetUserPayments(userUUID)
	if err != nil {
...
func (h *PaymentHandler) RefundPayment(c *gin.Context) {
	...
	if err := h.paymentService.RefundPayment(id, req.Amount, req.Reason); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *PaymentHandler) GetWallet(c *gin.Context) {
	...
	wallet, err := h.walletService.GetWallet(userUUID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *PaymentHandler) AddFunds(c *gin.Context) {
	...
	var req models.AddToWalletRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	if err := h.walletService.AddFunds(userUUID, req.Amount, req.Description); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *PaymentHandler) WithdrawFunds(c *gin.Context) {
	userID, exists := c.Get("user_id")
	if !exists {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "unauthorized"})
		return
	...
	var req models.WalletWithdrawRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	if err := h.walletService.WithdrawFunds(userUUID, req.Amount, req.Description); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "funds withdrawn successfully"})
}
...
func (h *PaymentHandler) GetTransactions(c *gin.Context) {
	...
	transactions, err := h.walletService.GetTransactions(userUUID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
